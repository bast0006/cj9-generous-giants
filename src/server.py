...
class GameManager:
    ...
    def create_room(self, pid: int):
        ...
        if existing_rid is None:
            ...
            self.room_count += 1
            return f"ServM:Welcome to room {rid}"

        else:
            print(f'Server message: Player {pid} already in room {existing_rid}')
            return f"ServM:Already in room {existing_rid}. Please leave room before creating new room"

    async def join_room(self, pid: int):
        """Join room"""
        ...
        if existing_rid == -1:
            ...
            print(f'Server message: Player {pid} already in room {existing_rid}')
            return f"ServM:Player {pid} already in room {existing_rid}"

        rid = self.assign_random_room()
        if rid is None:
            response = self.create_room(pid)
            return response
        else:
            ...
            if room is None:
                ...
                # f'Player {pid} added to room {rid}'
                return f'ServM:Player {pid} added to room {rid}'

    async def send_messages_in_chat(self, pid, message: str):
        """Send message to all players in chat"""
        ...
        if rid == -1:
            ...
            return "ServM:Join room to send messages"
    ...
    async def leave_room(self, pid: int):
        ...
        if rid == -1 or rid is None:
            ...
            return 'ServM:Join room to leave room'
    ...
    async def remove_player(self, player_id: int):
        ...
        if rid is None:
            ...
            return "ServM:Player not found"

    def assign_random_room(self):
        """Assign random room to player"""
        candidates = [rid for rid in self.rooms.keys() if self.rooms[rid].room_size < self.rooms[rid].max_room_size]
        if len(candidates) == 0:
            return None
        else:
    ...
    def see_all_rooms(self):
        ...
        if len(self.rooms) == 0:
            print("Server Message: No rooms found")
            return "ServM:No rooms found"
            ...
            try:
                ...
                match message:
                    ...
                    case 'Join room':
                        await websocket.send('Enter Player ID')
                        pid = await websocket.recv()
                        output = await self.join_room(int(pid))
                        await websocket.send(output)
